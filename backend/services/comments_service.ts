...
import { Comment, Reaction, SortType, User } from '../types';
import { hotCommentScore } from './util';

const sort_comp = (a: any, b: any, asc = 1) => {
    if(a < b) return -asc;
    else if(a > b) return asc;
    else return 0;
}

const sort_fns = {
    [SortType.New]: (a: Comment, b: Comment) => {
        return sort_comp(a.created_at, b.created_at, -1);
    },
    [SortType.Hot]: (a: Comment, b: Comment) => {
        return sort_comp(hotCommentScore(a), hotCommentScore(b), -1);
    },
...
function get_comments(url: string, parent_id: number | null, start: number, limit: number, sort_type = SortType.Hot){
    return new Promise((resolve, reject) => {
        get_website_id(url)
        .then(site_id => {
            mysql_db.pquery(comments_sql.get_comments(site_id, parent_id, start, limit), reject, (comments: Comment[]) => {
                ...
                .then((comments: Comment[]) => {
                    comments.sort(sort_fns[sort_type]);
                    return comments;
                })
                .then(resolve)
